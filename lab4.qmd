...
```{r}
### Exploring the abundance of Coastal Giant Salamander

and_vertebrates |> 
  filter(species == "Coastal giant salamander") |> 
  count(unittype)

### Preparing for chi-square test by creating a contingency table
## Null hypothesis: forest type and channel unit are independent 
...
### Testing assumptions
## Chi-square assumption: In the null hypothesis, all frequencies have equal probability

chisq.test(salamander_table)

### Conclusions
## The chi-square test resulted in a very small p value (< 2.2e-16), indicating a significant relationship between forest type and channel unit. Thus, we reject the null hypothesis.
```
...
```{r}
### Visualizing the differences in weight among forest type with a boxplot. 
...
salamander_clean |> 
  ...
  ggpubr::ggviolin(x = "section", 
    ...
    # Results: The box plot shows little variation but our t-test will help verify this using statistical analysis. 

### Null hypothesis: Species biomass for the Coastal giant salamander is not affected by forest type.  

### Testing assumptions
## t-test assumption 1: The variance of each group is equal

clearcut_weight <- salamander_clean |> 
  filter(section == "CC") |> 
  pull(weight_g)

oldgrowth_weight <- salamander_clean |> 
  filter(section == "OG") |> 
  pull(weight_g)

var.test(clearcut_weight, oldgrowth_weight)

    # Results: The results of our variance test suggest that the variance for old growth and clear cut weight are significantly different, based on the very small p value (1.439e-7). This assumption for our t-test is not met. 

## t-test assumption 2: The data's distribution is normal 

ggpubr::ggarrange(ggpubr::gghistogram(cc_weight, main = "Clear Cut"), 
                  ggpubr::gghistogram(og_weight, main = "Old Growth"))

    # Results: The results of our distribution visualization show that our data are very right skewed. This assumption for our t-test is not met. 

## Log transformation: 

var.test(log(clearcut_weight), log(oldgrowth_weight))

    # Results: Our p value is now much larger, indicating support for the null that the variances of log-transformed data are equal. So we can use the default t.test() test which assumes equal variances, but only on a log transformed weight variable.

### t-test

t.test(log(salamander_clean$weight_g) ~ salamander_clean$section, var.equal = TRUE)

    # Results: Given the very small p-value (< 2.2e-16) we can reject our null hypothesis that species biomass for the Coastal giant salamander is not affected by forest type. We can see that the Coastal giant salamander weight was observed to be significantly higher in clear cut forests compared to old growth forests.
...
gghistogram(sally_clean$weight_g, title = "Weight"))
    ...
    # Result: They both look pretty skewed, therefore likely not normally distributed. We will test the normal distribution statistically using the shapiro test. 
...
shapiro.test(q$weight_g)
    ...
    ## Overall results: We reject the Shapiro-Wilke null hypothesis due to our significant p-values and can conclude that our variables are not normally distributed.  
...
### Looking at the correlation visually. 
...
# Findings: We conducted an ANOVA test to see whether there were significant differences in crab size (measured as carapace width) across the three sites (GTM, DB, and PIE). The results of the ANOVA showed a significant effect of site on carapace width (p < 2e-16), indicating that at least one site differed significantly from the others in terms of crab size.Next, a Tukey's HSD post-hoc test was performed. The post-hoc analysis showed that: PIE and DB, and GTM and DB varied significantly in crab size with p-values of 2.9e-06 and 3.0e-07, respectively. These results suggest that crab size varies across the sites.
```
...
### Building a linear model for water temperature variation's effect on size 
pie_lm <- lm(size ~ water_temp_sd, data = pie_crab)

#view the results of the linear model
summary(pie_lm)

    # Results: This model results in a relatively large p-value (0.528), indicating that water temperature does not have a significant effect on crab size. We fail to reject the null hypothesis. 
...
```{r}
### Null hypothesis: Latitude, variation in air temperature, and variation in water temperature have no effect on crab size. 

### Checking for correlations among the three predictor variables

pie_crab |> 
  dplyr::select(latitude, air_temp_sd, water_temp_sd) |> 
  cor()

    # Results: All variables are not very correlated with each other (correlation coefficients less than +/- 0.8) and we will include all three variables in our model. 

### Creating a multiple linear regression model with the effects of latitude, air temperature, and variation in water temperature on size. 

pie_mlm <- lm(size ~ latitude + air_temp_sd + water_temp_sd, data = pie_crab)

summary(pie_mlm)

    # Results: This multiple regression model has an overall p-value that is very low (< 2.2e-16), and shows that each included variable has a significant impact on crab size. We fail to reject the null hypothesis.
```
